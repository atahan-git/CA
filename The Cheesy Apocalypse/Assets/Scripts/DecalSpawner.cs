...
public class DecalSpawner : MonoBehaviour {
	...
	void Start () {
		...
		for (int i = 0; i <= am; i++) {
			Instantiate(toSpawn[Random.Range(0,toSpawn.Length)],new Vector3(transform.position.x + Random.Range(-area.x/2f,area.x/2f), transform.position.y, transform.position.z + Random.Range(-area.z/2f,area.z/2f)), Quaternion.identity);
		}
	}
